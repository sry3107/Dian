...
if (!empty($query)) {
    $stmt = $conn->prepare("SELECT k.tanggal_kegiatan, k.waktu_kegiatan, mu.nama_mubaligh, mu.no_telepon_mubaligh 
        FROM jadwal j
        INNER JOIN kegiatan k ON j.id_kegiatan = k.id_kegiatan
        INNER JOIN mubaligh mu ON j.id_mubaligh = mu.id_mubaligh
        WHERE j.kode_jadwal = 'JKJ' AND mu.nama_mubaligh LIKE ?
...
<head>
    ...
    <title>Jadwal - Khutbah Jum'at</title>
    ...
    <section class="card py-16">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-2xl font-bold mb-8">Jadwal Khutbah Jum'at</h2>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <!-- Tombol Print Jadwal Khutbah Jum'at -->
                    <a href="cetak_khutbah_jumat.php" class="text-white bg-green-500 hover:bg-green-600 flex items-center gap-2 rounded-full py-2 px-4 shadow-lg" title="Cetak Daftar Khutbah Jum'at">
                        <i class="fas fa-print"></i>
                        <span>Daftar</span>
                    </a>
                    <!-- Tombol Print Laporan Jadwal Khutbah Jum'at Tahunan -->
                    <a href="laporan_jadwal_khutbah_jumat_tahunan.php" class="text-white bg-green-500 hover:bg-green-600 flex items-center gap-2 rounded-full py-2 px-4 shadow-lg" title="Cetak Laporan Khutbah Jum'at Tahunan">
                        <i class="fas fa-print"></i>
                        <span>Laporan</span>
                    </a>
                </div>
            ...
            <table class="min-w-full bg-white border border-gray-200">
                <thead>
                    <tr>
                        ...
                        <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Nama Mubaligh</th>
                    ...
                    <?php if (!empty($data_jadwal)): ?>
                        <?php foreach ($data_jadwal as $index => $jadwal): ?>
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($jadwal['waktu_kegiatan'] ?? ''); ?></td>
                                <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($jadwal['nama_mubaligh'] ?? ''); ?></td>
                                <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($jadwal['no_telepon_mubaligh'] ?? ''); ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="8" class="text-center py-4 text-gray-500">
                                <?php echo !empty($query) ? "(Tidak ada hasil untuk pencarian \"" . htmlspecialchars($query) . "\")" : "(Data khutbah jum'at tidak ditemukan!)"; ?>
                            </td>
                        </tr>
                    <?php endif; ?>
                </tbody>
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                // Klik akan mengarahkan ke khutbah_jumat_pengurus.php
                                resultDiv.addEventListener("click", function () {
                                    window.location.href = `khutbah_jumat_pengurus.php?query=${encodeURIComponent(query)}`;
                                });
