...
if (!empty($query)) {
    $stmt = $conn->prepare("SELECT * FROM kegiatan WHERE nama_kegiatan LIKE ? AND kode_kegiatan='KBL' ORDER BY tanggal_kegiatan ASC");
    $search = "%$query%";
    $stmt->bind_param("s", $search);
} else {
    $stmt = $conn->prepare("SELECT * FROM kegiatan WHERE kode_kegiatan='KBL' ORDER BY tanggal_kegiatan ASC");
}
    ...
    <section class="py-16">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-2xl font-bold mb-8">Daftar Kegiatan Bulanan</h2>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <!-- Tombol Print Kegiatan Bulanan -->
                    <a href="cetak_kegiatan_bulanan.php" class="text-white bg-green-500 hover:bg-green-600 flex items-center gap-2 rounded-full py-2 px-4 shadow-lg" title="Cetak Daftar Kegiatan">
                        <i class="fas fa-print"></i>
                        <span>Daftar</span>
                    </a>
                    <!-- Tombol Print Laporan Bulanan -->
                    <a href="laporan_kegiatan_bulanan.php" class="text-white bg-green-500 hover:bg-green-600 flex items-center gap-2 rounded-full py-2 px-4 shadow-lg" title="Cetak Laporan Bulanan">
                        <i class="fas fa-print"></i>
                        <span>Laporan</span>
                    </a>
                </div>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="text-center py-4 text-gray-500">
                                    <?php echo !empty($query) ? "(Tidak ada hasil untuk pencarian \"" . htmlspecialchars($query) . "\")" : "(Data kegiatan bulanan tidak ditemukan!)"; ?>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    const searchInput = document.getElementById("search-input");
        ...
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                ...
                fetch(`cari_kegiatan_bulanan.php?query=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                // Klik akan mengarahkan ke kegiatan_bulanan_pengurus.php
                                resultDiv.addEventListener("click", function () {
                                    window.location.href = `kegiatan_bulanan_pengurus.php?query=${encodeURIComponent(query)}`;
                                });
