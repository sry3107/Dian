...
if ($resultAkun->num_rows > 0) {
    ...
    $queryMubaligh = $conn->prepare("SELECT id_mubaligh, id_akun, COALESCE(nama_mubaligh, '') AS nama_mubaligh, COALESCE(alamat_mubaligh, '') AS alamat_mubaligh, COALESCE(no_telepon_mubaligh, '') AS no_telepon_mubaligh, COALESCE(created_at, '') AS created_at, COALESCE(updated_at, '') AS updated_at FROM mubaligh WHERE id_akun = ?");
...
<body>
    ...
    <!-- Detail Profil Mubaligh -->
    <section class="card py-16">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-2xl font-bold">Detail Profil Mubaligh</h2>
                <div class="flex items-center space-x-4">
                    <!-- Icon Settings -->
                    <a href="pengaturan_akun_mubaligh.php" class="text-white bg-gray-500 hover:text-gray-300 rounded-full p-3 shadow-lg" title="Pengaturan Akun">
                    ...
                    <?php
                    // Cek apakah data mubaligh dengan id_akun ini sudah ada
                    $queryCheck = "SELECT COUNT(*) AS total FROM mubaligh WHERE id_akun = ?";
                    ...
                    // Tampilkan tombol "Input Data" hanya jika data mubaligh belum ada
                    if ($dataCheck['total'] == 0): ?>
                        <!-- Icon Input Data -->
                        <a href="edit_profil_mubaligh.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Input Data">
            ...
            <?php
            // Ambil data mubaligh untuk ditampilkan
            $query = "SELECT * FROM mubaligh WHERE id_akun = ?";
            ...
            ?>
            <!-- Card Profil Mubaligh -->
            <div class="card-hover bg-white rounded-xl overflow-hidden mb-4 shadow-md">
                <a href="edit_profil_mubaligh.php?id=<?php echo $data['id_mubaligh']; ?>" class="block w-full h-full">
                    <div class="p-6">
                            ...
                            <table class="table-auto w-full">
                                <tbody>
                                    <tr class="border-b">
                                        ...
                                        <td class="py-2">: <?php echo htmlspecialchars($data['nama_mubaligh']); ?></td>
                                    </tr>
                                    <tr class="border-b">
                                        <th class="text-left py-2 pr-4 font-medium">
                                            <i class="fas fa-map-marker-alt"></i> Alamat
                                        </th>
                                        <td class="py-2">: <?php echo htmlspecialchars($data['alamat_mubaligh']); ?></td>
                                    </tr>
                                    <tr>
                                        <th class="text-left py-2 pr-4 font-medium">
                                            <i class="fas fa-phone-alt"></i> Nomor Telepon
                                        </th>
                                        <td class="py-2">: <?php echo htmlspecialchars($data['no_telepon_mubaligh']); ?></td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="text-center">
                <p class="text-gray-500">Tidak ada data mubaligh tersedia.</p>
            </div>
            <?php endif; ?>
        </div>
    </section>
