...
if (!empty($start_date) && !empty($end_date)) {
    ...
    $stmt = $conn->prepare("SELECT * FROM kegiatan WHERE tanggal_kegiatan BETWEEN ? AND ? ORDER BY tanggal_kegiatan ASC");
    $stmt->bind_param("ss", $start_date, $end_date);
} else if (!empty($query)) {
    // Jika ada query pencarian
    $stmt = $conn->prepare("SELECT * FROM kegiatan WHERE nama_kegiatan LIKE ? ORDER BY tanggal_kegiatan ASC");
    $search = "%$query%";
    $stmt->bind_param("s", $search);
} else {
    // Tampilkan semua data
    $stmt = $conn->prepare("SELECT * FROM kegiatan ORDER BY tanggal_kegiatan ASC");
}
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($deleteQuery->execute()) {
        echo "<script>alert('Data kegiatan berhasil dihapus!'); window.location.href='manage_kegiatan.php';</script>";
    } else {
        echo "<script>alert('Gagal menghapus data kegiatan!'); window.location.href='manage_kegiatan.php';</script>";
    }
    exit;
}
?>
...
<head>
    ...
    <title>Kegiatan - Kegiatan Masjid</title>
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-6">
                ...
                <div class="flex flex-col lg:flex-row gap-4 items-center">
                    ...
                    ?>
                    <a href="<?= $isFilterActive ? "laporan_kegiatan.php?start_date=" . htmlspecialchars($_GET['start_date']) . "&end_date=" . htmlspecialchars($_GET['end_date']) : '#' ?>"
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="tambah_kegiatan.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Tambah Kegiatan">
                            ...
                            <?php foreach ($data_kegiatan as $index => $row): ?>
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="py-4 px-6 border-b border-gray-200 text-left">
                                        <a href="edit_kegiatan.php?id=<?php echo $row['id_kegiatan']; ?>" class="text-blue-600 hover:text-blue-800">Edit</a>
                                        <span class="mx-1">|</span>
                                        <a href="manage_kegiatan.php?id=<?php echo $row['id_kegiatan']; ?>&action=delete" class="text-red-600 hover:text-red-800" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?');">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
    ...
    <script> 
        ...
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                ...
                fetch(`cari_kegiatan.php?query=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                // Klik akan mengarahkan ke manage_kegiatan.php
                                resultDiv.addEventListener("click", function () {
                                    window.location.href = `manage_kegiatan.php?query=${encodeURIComponent(query)}`;
                                });
