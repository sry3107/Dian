...
if (!empty($query)) {
    $stmt = $conn->prepare("SELECT * FROM mubaligh WHERE nama_mubaligh LIKE ? AND created_at IS NOT NULL ORDER BY created_at ASC");
    $search = "%$query%";
    $stmt->bind_param("s", $search);
} else {
    $stmt = $conn->prepare("SELECT * FROM mubaligh");
}

$stmt->execute();
$result = $stmt->get_result();

// Cek apakah data mubaligh ada
$data_mubaligh = $result->num_rows > 0 ? $result->fetch_all(MYSQLI_ASSOC) : [];
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($deleteQuery->execute()) {
        ...
        echo "<script>alert('Gagal menghapus data mubaligh!');</script>";
    }
    exit;
}
?>
    ...
    <section class="card py-16">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-2xl font-bold">Daftar Mubaligh</h2>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <!-- Tombol Cetak Daftar Mubaligh -->
                    <a href="cetak_daftar_mubaligh.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Cetak Daftar Mubaligh">
                        <i class="fas fa-print mr-1 ml-1"></i>
                    </a>
                    <!-- Tombol Tambah Data Mubaligh -->
                    <a href="tambah_mubaligh.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Tambah Data Mubaligh">
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <table class="min-w-full bg-white">
                    ...
                    <tbody>
                        <?php if (!empty($data_mubaligh)): ?>
                            <?php foreach ($data_mubaligh as $index => $row): ?>
                                <tr class="hover:bg-gray-100">
                                    <td class="py-4 px-6 border-b border-gray-200"><?= $index + 1; ?>.</td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['nama_mubaligh'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['alamat_mubaligh'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['no_telepon_mubaligh'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200 text-left">
                                        <a href="manage_profil_mubaligh.php?id=<?php echo $row['id_mubaligh']; ?>" class="text-blue-600 hover:text-blue-800">Edit</a>
                                        <span class="mx-1">|</span>
                                        <a href="manage_mubaligh.php?id=<?php echo $row['id_mubaligh']; ?>&action=delete" class="text-red-600 hover:text-red-800" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini? Penghapusan akan menghilangkan data terkait mubaligh!');">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="text-center py-4 text-gray-500">
                                    <?php echo !empty($query) ? "(Tidak ada hasil untuk pencarian \"" . htmlspecialchars($query) . "\")" : "(Data mubaligh tidak ditemukan!)"; ?>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    <script> 
        ...
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                ...
                fetch(`cari_mubaligh.php?query=${encodeURIComponent(query)}`)
                        ...
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                resultDiv.addEventListener("click", function () {
                                    window.location.href = `manage_mubaligh.php?query=${encodeURIComponent(query)}`;
                                });
