...
if ($resultAkun->num_rows > 0) {
    ...
    $current_user_id = $akunData['id_akun']; // Mendapatkan id_akun
} else {
...
if (!empty($query)) {
    $stmt = $conn->prepare("SELECT * FROM akun WHERE username LIKE ? ORDER BY created_at ASC");
    $search = "%$query%";
    $stmt->bind_param("s", $search);
} else {
    $stmt = $conn->prepare("SELECT * FROM akun ORDER BY created_at ASC");
}

$stmt->execute();
$result = $stmt->get_result();
$data_akun = $result->num_rows > 0 ? $result->fetch_all(MYSQLI_ASSOC) : [];
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $id_akun_to_delete = intval($_GET['id']); // Sanitasi ID untuk mencegah eksploitasi

    // Periksa apakah ID akun yang dihapus sama dengan akun yang sedang login
    if ($id_akun_to_delete === $current_user_id) {
        $deleteQuery = $conn->prepare("DELETE FROM akun WHERE id_akun = ?");
        $deleteQuery->bind_param("i", $id_akun_to_delete);

        if ($deleteQuery->execute()) {
            echo "<script>alert('Akun Anda telah dihapus. Silahkan log in/registrasi ulang!'); window.location.href='login.php';</script>";
        } else {
            echo "<script>alert('Gagal menghapus akun Anda!'); window.location.href='manage_akun.php';</script>";
        }
    } else {
        $deleteQuery = $conn->prepare("DELETE FROM akun WHERE id_akun = ?");
        $deleteQuery->bind_param("i", $id_akun_to_delete);
        ...
        if ($deleteQuery->execute()) {
            echo "<script>alert('Data akun berhasil dihapus!'); window.location.href='manage_akun.php';</script>";
        } else {
            echo "<script>alert('Gagal menghapus data akun!'); window.location.href='manage_akun.php';</script>";
        }
    }
    ...
    <section class="card py-16">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-2xl font-bold">Daftar Akun Pengguna</h2>
                    <div class="flex items-center gap-4">
                    <!-- Form Pencarian -->
                    <form action="" method="GET" class="relative">
                        <input type="search" name="query" placeholder="Masukkan Username..." 
                            class="w-80 pl-10 pr-4 py-2 rounded-full border border-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500 text-sm" 
                    ...
                    <!-- Tombol Cetak Daftar Akun -->
                    <a href="cetak_akun_pengguna.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Cetak Daftar Akun Pengguna">
                ...
                <table class="min-w-full bg-white border border-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Username</th>
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Email</th>
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Role</th>
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Status</th>
                        ...
                        <?php if (!empty($data_akun)): ?>
                            <?php foreach ($data_akun as $index => $akun): ?>
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="py-4 px-6 border-b border-gray-200"><?= htmlspecialchars($akun['username']); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?= htmlspecialchars($akun['email']); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?= htmlspecialchars($akun['role']); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?= htmlspecialchars($akun['status']); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200 text-left">
                                        <a href="manage_akun.php?id=<?= $akun['id_akun']; ?>&action=delete" class="text-red-600 hover:text-red-800" onclick="return confirm('Yakin ingin menghapus akun ini? Penghapusan akan menghilangkan data terkait akun!');">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="text-center py-4 text-gray-500">
                                    <?php echo !empty($query) ? "(Tidak ada hasil untuk pencarian \"" . htmlspecialchars($query) . "\")" : "(Data akun tidak ditemukan!)"; ?>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    <script> 
        ...
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                ...
                fetch(`cari_akun.php?query=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                resultDiv.innerHTML = `
                                    <h3 class="text-sm font-medium text-gray-700">${item.username}</h3>
                                `;

                                // Klik akan mengarahkan ke manage_akun.php
                                resultDiv.addEventListener("click", function () {
                                    window.location.href = `manage_akun.php?query=${encodeURIComponent(query)}`;
                                });
