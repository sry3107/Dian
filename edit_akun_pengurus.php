...
$username = $_SESSION['username'] ?? null; // Cek session username
if (!$username) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_account'])) {
    ...
    $hashed_password = !empty($password) ? password_hash($password, PASSWORD_DEFAULT) : $data_akun['password'];

    // Cek apakah username atau email yang dimasukkan sudah ada di database (selain data yang sedang diupdate)
    ...
    if (empty($username_message) && empty($email_message)) {
        // Proses update jika tidak ada pesan error
    ...
    <button type="button" onclick="confirmDelete(<?php echo $data_akun['id_akun']; ?>)" class="mt-4">Delete</button> 
        <script>
            function confirmDelete(id){
                ...
                if (confirmation) {
                    window.location.href='edit_akun_pengurus.php?id=<?php echo $data_akun['id_akun']; ?>&action=delete'
                    } else{
                        window.location.href='pengaturan_akun_pengurus.php';
                    }
                }
        </script>
    <button type="button" onclick="location.href='pengaturan_akun_pengurus.php'" class="mt-4">Cancel</button>
</div>
</body>
</html> 