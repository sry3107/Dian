...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Jadwal berhasil ditambahkan!'); window.location.href='manage_jadwal.php';</script>";
    } else {
            ...
            <div class="form-group">
                ...
                <select name="id_mubaligh" id="id_mubaligh" required>
                    ...
                    while ($row = $mubalighQuery->fetch_assoc()) {
                        echo "<option value='" . htmlspecialchars($row['id_mubaligh']) . "'>" . htmlspecialchars($row['nama_mubaligh']) . "</option>";
                    }
                    ?>
                </select>
            </div>
