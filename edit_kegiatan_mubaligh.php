...
if (!$id_kegiatan || !filter_var($id_kegiatan, FILTER_VALIDATE_INT)) {
    echo "<script>alert('ID Kegiatan tidak valid!'); window.location.href='kegiatan_mubaligh.php';</script>";
    exit;
}
...
if ($result->num_rows > 0) {
    ...
    echo "<script>alert('Data tidak ditemukan!'); window.location.href='kegiatan_mubaligh.php';</script>";
    exit;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($updateQuery->execute()) {
        echo "<script>alert('Data kegiatan berhasil diperbarui!'); window.location.href='kegiatan_mubaligh.php';</script>";
    } else {
...
<body>
    <div class="container">
        ...
        <form action="" method="post">
            <div class="form-group">
                ...
                <select name="nama_kegiatan" id="nama_kegiatan" required>
                    ...
                    $kegiatanQuery = $conn->query("SELECT DISTINCT nama_kegiatan FROM kegiatan WHERE nama_kegiatan IN ('Khutbah Jum\'at', 'Pengajian', 'Kajian', 'Tabligh Akbar')");
        ...
        <button type="button" onclick="location.href='kegiatan_mubaligh.php'">Cancel</button>
    </div>
