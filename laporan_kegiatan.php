...
$awal_tahun = isset($_GET['start_date']) ? $_GET['start_date'] : date('Y-01-01');
$akhir_tahun = isset($_GET['end_date']) ? $_GET['end_date'] : date('Y-12-31');
...
$query = $conn->prepare("SELECT 
    ...
    WHERE k.tanggal_kegiatan BETWEEN ? AND ?
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, 'LAPORAN DAFTAR KEGIATAN MASJID', 0, 1, 'C');
...
if (!empty($data_kegiatan)) {
    ...
    $pdf->Cell(0, 10, 'Tidak ada data kegiatan untuk periode ini.', 1, 1, 'C');
}
