...
if (!empty($query)) {
    $stmt = $conn->prepare(
        "SELECT id_pengurus, nama_pengurus, jabatan_pengurus, alamat_pengurus, no_telepon_pengurus
        FROM pengurus 
        WHERE nama_pengurus LIKE ? 
...
$data_pengurus = $result->num_rows > 0 ? $result->fetch_all(MYSQLI_ASSOC) : [];
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    ...
    if ($deleteQuery->execute()) {
        ...
        echo "<script>alert('Gagal menghapus data pengurus!');</script>";
    }
    exit;
}
?>
...
<body>
    ...
    <section class="card py-16">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex justify-between items-center mb-8">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <form action="" method="GET" class="relative">
                        <input type="search" name="query" placeholder="Masukkan Nama Pengurus..." 
                    ...
                    <!-- Tombol Cetak Daftar Pengurus -->
                    <a href="cetak_daftar_pengurus.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Cetak Daftar Pengurus">
                        <i class="fas fa-print mr-1 ml-1"></i>
                    </a>
                    <!-- Tombol Tambah Data Pengurus -->
                    <a href="tambah_pengurus.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Tambah Data Pengurus">
            ...
            <!-- Display profile data in a table format -->
                ...
                <table class="min-w-full bg-white">
                    <thead>
                        <tr>
                            ...
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Nama</th>
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Jabatan</th>
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Alamat</th>
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">No. Telepon</th>
                    ...
                    <tbody>
                        <?php if (!empty($data_pengurus)): ?>
                            <?php foreach ($data_pengurus as $index => $row): ?>
                                <tr class="hover:bg-gray-100">
                                    <td class="py-4 px-6 border-b border-gray-200"><?= $index + 1; ?>.</td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['nama_pengurus'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['jabatan_pengurus'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['alamat_pengurus'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['no_telepon_pengurus'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200 text-left">
                                        <a href="manage_profil_pengurus.php?id=<?php echo $row['id_pengurus']; ?>" class="text-blue-600 hover:text-blue-800">Edit</a>
                                        <span class="mx-1">|</span>
                                        <a href="manage_pengurus.php?id=<?php echo $row['id_pengurus']; ?>&action=delete" class="text-red-600 hover:text-red-800" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini? Penghapusan akan menghilangkan data terkait pengurus!');">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" class="text-center py-4 text-gray-500">
                                    <?php echo !empty($query) ? "(Tidak ada hasil untuk pencarian \"" . htmlspecialchars($query) . "\")" : "(Data pengurus tidak ditemukan!)"; ?>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    <script> 
        ...
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                ...
                fetch(`cari_pengurus.php?query=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                resultDiv.innerHTML = `
                                    <h3 class="text-sm font-medium text-gray-700">${item.nama_pengurus}</h3>
                                `;

                                // Klik akan mengarahkan ke manage_pengurus.php
                                resultDiv.addEventListener("click", function () {
                                    window.location.href = `manage_pengurus.php?query=${encodeURIComponent(query)}`;
                                });
