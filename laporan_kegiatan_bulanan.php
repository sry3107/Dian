...
$hari_ini = date('Y-m-d');
$awal_bulan = date('Y-m-01'); // Hari pertama bulan ini
$akhir_bulan = date('Y-m-t'); // Hari terakhir bulan ini
...
$query = $conn->prepare("SELECT 
    ...
    WHERE k.kode_kegiatan = 'KBL'
    AND k.tanggal_kegiatan BETWEEN ? AND ?
    ORDER BY k.tanggal_kegiatan ASC
");
$query->bind_param('ss', $awal_bulan, $akhir_bulan);
$query->execute();
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, 'LAPORAN KEGIATAN BULANAN', 0, 1, 'C');
        $this->SetFont('Arial', '', 12);
        $this->Cell(0, 5, $this->nama_masjid, 0, 1, 'C');
        $this->SetFont('Arial', 'IU', 11);
        $this->Cell(0, 5, $this->alamat_masjid, 0, 1, 'C');
        $this->SetFont('Arial', 'B', 11);
        $this->Cell(0, 7, 'Periode: ' . date('d M Y', strtotime($this->awal_bulan)) . ' - ' . date('d M Y', strtotime($this->akhir_bulan)), 0, 1, 'C');
        $this->Ln(8);
...
$pdf->awal_bulan = $awal_bulan;
$pdf->akhir_bulan = $akhir_bulan;
...
if (!empty($data_kegiatan)) {
    ...
    $pdf->Cell(0, 10, 'Tidak ada data kegiatan untuk bulan ini.', 1, 1, 'C');
}
