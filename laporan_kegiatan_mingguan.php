...
// Menentukan awal dan akhir minggu
$hari_ini = date('Y-m-d');
$awal_minggu = date('Y-m-d', strtotime('monday this week', strtotime($hari_ini)));
$akhir_minggu = date('Y-m-d', strtotime('sunday this week', strtotime($hari_ini)));

// Query untuk mengambil data kegiatan dalam range tanggal
$query = $conn->prepare("SELECT 
    ...
    WHERE k.kode_kegiatan = 'KMG'
    AND k.tanggal_kegiatan BETWEEN ? AND ?
    ORDER BY k.tanggal_kegiatan ASC
");
$query->bind_param('ss', $awal_minggu, $akhir_minggu);
$query->execute();
...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(0, 10, 'LAPORAN KEGIATAN MINGGUAN', 0, 1, 'C');
        $this->SetFont('Arial', '', 12);
        $this->Cell(0, 5, $this->nama_masjid, 0, 1, 'C');
        $this->SetFont('Arial', 'IU', 11);
        $this->Cell(0, 5, $this->alamat_masjid, 0, 1, 'C');
        $this->SetFont('Arial', 'B', 11);
        $this->Cell(0, 7, 'Periode: ' . date('d M Y', strtotime($this->awal_minggu)) . ' - ' . date('d M Y', strtotime($this->akhir_minggu)), 0, 1, 'C');
        $this->Ln(8);
...
$pdf->awal_minggu = $awal_minggu;
$pdf->akhir_minggu = $akhir_minggu;
...
if (!empty($data_kegiatan)) {
    ...
    foreach ($data_kegiatan as $kegiatan) {
        ...
        $pdf->Cell(20, 10, $kegiatan['waktu_kegiatan'], 1, 0, 'C');
        $pdf->Cell(70, 10, $kegiatan['nama_kegiatan'], 1, 0, 'L');
        $pdf->Cell(55, 10, $kegiatan['tempat_kegiatan'], 1, 1, 'C');
    }
} else {
    $pdf->Cell(0, 10, 'Tidak ada data kegiatan untuk minggu ini.', 1, 1, 'C');
}
