...
if (!empty($query)) {
    // Query pencarian berdasarkan query dan nama_kegiatan yang spesifik
    $stmt = $conn->prepare("SELECT * FROM kegiatan WHERE nama_kegiatan LIKE ? AND nama_kegiatan IN ('Khutbah Jum\'at', 'Pengajian', 'Kajian', 'Tabligh Akbar') AND created_at IS NOT NULL ORDER BY tanggal_kegiatan ASC");
    $search = "%$query%";
    $stmt->bind_param("s", $search);
} else {
    // Query untuk menampilkan semua kegiatan tertentu jika tidak ada query pencarian
    $stmt = $conn->prepare("SELECT * FROM kegiatan WHERE nama_kegiatan IN ('Khutbah Jum\'at', 'Pengajian', 'Kajian', 'Tabligh Akbar') AND created_at IS NOT NULL ORDER BY tanggal_kegiatan ASC");
}
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $id_kegiatan = $_GET['id'];
    $deleteQuery = $conn->prepare("DELETE FROM kegiatan WHERE id_kegiatan = ?");
    $deleteQuery->bind_param("i", $id_kegiatan);

    if ($deleteQuery->execute()) {
        echo "<script>alert('Data kegiatan berhasil dihapus!'); window.location.href='kegiatan_mubaligh.php';</script>";
    } else {
        echo "<script>alert('Gagal menghapus data kegiatan!'); window.location.href='kegiatan_mubaligh.php';</script>";
    }
    exit;
}
?>
...
<head>
    ...
    <title>Kegiatan - Kelola Kegiatan</title>
    ...
    <section class="card py-16">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-2xl font-bold">Daftar Kegiatan Masjid</h2>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="tambah_kegiatan_mubaligh.php" class="text-white bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg" title="Tambah Kegiatan">
                ...
                <table class="min-w-full bg-white border border-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="py-3 px-6 bg-gray-100 font-semibold text-sm text-gray-900 border-b border-gray-200 text-left">Kode</th>
                        ...
                        <?php if (!empty($data_kegiatan)): ?>
                            <?php foreach ($data_kegiatan as $index => $row): ?>
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['kode_kegiatan'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['nama_kegiatan'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['deskripsi_kegiatan'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['tempat_kegiatan'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['waktu_kegiatan'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200"><?php echo htmlspecialchars($row['tanggal_kegiatan'] ?? ''); ?></td>
                                    <td class="py-4 px-6 border-b border-gray-200 text-left">
                                        <a href="edit_kegiatan_mubaligh.php?id=<?php echo $row['id_kegiatan']; ?>" class="text-blue-600 hover:text-blue-800">Edit</a>
                                        <span class="mx-1">|</span>
                                        <a href="kegiatan_mubaligh.php?id=<?php echo $row['id_kegiatan']; ?>&action=delete" class="text-red-600 hover:text-red-800" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?');">Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="8" class="text-center py-4 text-gray-500">
                                    <?php echo !empty($query) ? "(Tidak ada hasil untuk pencarian \"" . htmlspecialchars($query) . "\")" : "(Data kegiatan tidak ditemukan!)"; ?>
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
    ...
    const searchInput = document.getElementById("search-input");
        ...
        searchInput.addEventListener("input", function () {
            ...
            if (query.length > 0) {
                ...
                fetch(`cari_kegiatan_mubaligh.php?query=${encodeURIComponent(query)}`)
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            ...
                            data.forEach(item => {
                                ...
                                // Klik akan mengarahkan ke kegiatan_mubaligh.php
                                resultDiv.addEventListener("click", function () {
                                    window.location.href = `kegiatan_mubaligh.php?query=${encodeURIComponent(query)}`;
                                });
